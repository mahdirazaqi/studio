...
### K4 — Destructive retry, with the quota check *after* the delete
...
The bot only checks *identity* (phone-linked). It performs **no** `ActionEnum` checks. Any
...
YouTube upload errors flip the job to `Error` but the *reason* is only in server logs;
