...
- **Templates get edited.** An FK to a live Template row would show the *current*
...
- **Composition/output semantics** the Worker used are whatever the Template said *then*.
...
> *Consequence of JSONB:* simplest, naturally immutable, easy to serve to the Worker;
> harder to query/aggregate across jobs. *Consequence of columns:* queryable; more schema
...
> **`OPEN DECISION` — do we copy the media *bytes* for critical inputs?** The snapshot
> copies file *metadata* always. Should it also copy the *bytes* of input files (so a
> historical Job can still preview/re-render even after the File is purged)? *Consequence
> of copying bytes:* true immutability, re-render always possible, but storage cost
> multiplies and undermines File hard-deletion savings. *Consequence of metadata only:*
...
- [ ] Historical Job UI reads the snapshot; it may *additionally* show "this Template was
