...
> `User.phone`/`User.telegramUserId` and `TelegramWizardState` moved to **implemented** in
> Phase 8 (see [../integrations/telegram.md](../integrations/telegram.md), ADR-0035–0038).
...
> **Resolved for delivery outcomes (Phase 9, ADR-0039):** child rows (`DeliveryAttempt`),
> one per attempt per provider — never folded into `Job.snapshot`'s JSONB, and never
> updated in place across retries (a retry inserts a new row with the next
> `attemptNumber`).
...
| `User.phone` unique when not null — **implemented**                                                                                    | Makes a Telegram phone match never ambiguous (ADR-0036, resolves OD-06).                                                                                                       |
| `User.telegramUserId` unique when not null — **implemented**                                                                           | One Telegram account ↔ one User.                                                                                                                                               |
...
| `TelegramWizardState (telegramUserId)` unique, `(userId)` unique — **implemented**                                                     | Lookup by Telegram id; at most one active conversation per Studio User. TTL uses the existing `updatedAt` column, checked lazily on read — no separate sweep index needed yet. |
