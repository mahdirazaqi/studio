...
| Render Worker        | `WorkerApiKey` credential (ADR-0004, ADR-0040) | A dedicated non-user principal with a fixed, narrow capability set (Worker API only), scoped to one or more Departments |

The Worker principal is **not** a `User`; it does not have a single Department the way a
`User` does, but its credential (`WorkerApiKey`) is scoped to one or more Departments
(ADR-0040, revises the earlier "no Department" framing) and can only call the Worker API
use cases (claim/progress/state/duration/result/input-upload) — and only on Jobs within
its own credential's Department scope. It cannot read the panel, list jobs arbitrarily,
or touch users/templates.
...
| Transfer a template to another department                                 | ⬛                              | ⬛                                       | ✅                                | **New, ADR-0040.** ADMIN-only, even for a MANAGER otherwise authorized to edit the template — a client-supplied `departmentId` from a non-ADMIN is silently ignored. Dependent File/YouTube references re-verified against the _target_ department; historical Jobs unaffected (`Job.departmentId` is copied at creation, never a live join). |
| Disable template                                                          | ⬛                              | 🟨                                       | ✅                                |                                                                                                                                                                                                                                                                                                                                               |
...
| Delete a Job Artifact manually                                            | ⬛                              | ⬛                                       | ⬛                                | **Nobody, ever** (implemented, Phase 9) — a `JOB_ARTIFACT` File is explicitly refused by `assertCanDeleteFile` for every role; its only deletion path is the system-internal `cleanupJobArtifacts` primitive.                                                                                                                                 |
...
| Retry job delivery (implemented, Phase 9)                                 | 🟨 (any in dept)                | 🟨 (any in dept)                         | ✅                                | `job:manage`, same floor as cancel/retry. Only from `ERROR`, with a rendered video and no already-succeeded delivery.                                                                                                                                                                                                                         |
| Delete job                                                                | ⬛                              | ⬛                                       | ⬛                                | **Nobody. Ever.** (ADR-0005)                                                                                                                                                                                                                                                                                                                  |
| **YouTube (implemented, Phase 9; ADMIN-only revised Phase 11, ADR-0040)** |                                 |                                          |                                   |                                                                                                                                                                                                                                                                                                                                               |
| Connect / disconnect a channel                                            | ⬛                              | ⬛                                       | ✅                                | **Revised, ADR-0040:** `youtube:manage` is now ADMIN-only (was `MANAGER+`, department-scoped) — connecting a channel and choosing its Department scope is system-wide infrastructure configuration.                                                                                                                                           |
| Edit a channel's Department scope                                         | ⬛                              | ⬛                                       | ✅                                | **New, ADR-0040.** Full replace of the assigned Departments (m2m — one channel may serve several departments now).                                                                                                                                                                                                                            |
| View connected channels                                                   | ⬛                              | ⬛                                       | ✅                                | Same capability as above — no separate view-only floor. USER/MANAGER never see the management list, only the picker below.                                                                                                                                                                                                                    |
| Pick a channel for a Template                                             | ⬛ (matches template:manage)    | 🟨                                       | ✅                                | Gated by `template:manage`, not `youtube:manage` — authoring a Template, not managing Targets. Filtered to channels assigned to the Template's own Department.                                                                                                                                                                                |
| **Worker API Keys (new, Phase 11, ADR-0040)**                             |                                 |                                          |                                   |                                                                                                                                                                                                                                                                                                                                               |
| Create / revoke / reactivate a key                                        | ⬛                              | ⬛                                       | ✅                                | `worker_key:manage`, ADMIN-only — the machine-identity counterpart to `user:manage`. Secret shown once at creation, never re-displayed.                                                                                                                                                                                                       |
| Edit a key's Department scope                                             | ⬛                              | ⬛                                       | ✅                                | Full replace of the assigned Departments. Takes effect on the Worker's next request — nothing cached.                                                                                                                                                                                                                                         |
| **Worker API**                                                            | —                               | —                                        | —                                 | Only the Worker principal (a `WorkerApiKey`, not a `User`). No human role can call it. Department-scoped by the authenticated key (ADR-0040) — see docs/integrations/worker-api.md §4.                                                                                                                                                        |
