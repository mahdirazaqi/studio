...
| Edit template                                 | ⬛ (confirmed, Phase 5 — OD-04) | 🟨                                       | ✅                                | Edits never alter existing Jobs (snapshots). **Can never change `departmentId`, for any role — structurally, not by convention (ADR-0042).** The edit schema has no `departmentId` field at all; a client-submitted one is stripped before the use case ever runs.                                                                                                  |
| Transfer a template to another department     | ⬛                              | ⬛                                       | ✅                                | **New, ADR-0040; split into its own operation, ADR-0042.** A separate, ADMIN-only `transferTemplateDepartment` — never part of Edit, even for a MANAGER otherwise authorized to edit the template. Dependent File references re-verified against the _target_ department; historical Jobs unaffected (`Job.departmentId` is copied at creation, never a live join). |
| Disable template                              | ⬛                              | 🟨                                       | ✅                                |                                                                                                                                                                                                                                                                                                                                                                     |
