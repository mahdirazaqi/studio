...
| Transfer a template to another department     | ⬛                              | ⬛                                       | ✅                                | **New, ADR-0040.** ADMIN-only, even for a MANAGER otherwise authorized to edit the template — a client-supplied `departmentId` from a non-ADMIN is silently ignored. Dependent File references re-verified against the _target_ department; historical Jobs unaffected (`Job.departmentId` is copied at creation, never a live join). |
| Disable template                              | ⬛                              | 🟨                                       | ✅                                |                                                                                                                                                                                                                                                                                                                                       |
