...
| **Render Worker** communication                                                                                 | **Route Handler** under `app/api/worker/**`                                                                                                              | **Implemented, Phase 7.** `app/api/worker/v1/jobs/{next,[jobId],[jobId]/state,progress,duration}`, each a thin `defineRouteHandler` over a Phase 6 use case, authenticated by `@/server/worker-auth` (a shared static `WORKER_API_KEY`, ADR-0032). See [worker-api.md](../integrations/worker-api.md).                                                                                                                                                                                                                                                                                    |
...
6. **The Worker API is versioned** — **Implemented, Phase 7:** a path segment,
   `/api/worker/v1/...` (resolves the OPEN DECISION on exact scheme). Internal Server
