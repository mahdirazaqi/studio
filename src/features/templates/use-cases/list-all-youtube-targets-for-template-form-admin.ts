...
/**
 * ADMIN-only, cross-department `CONNECTED` YouTube Target listing — mirrors
 ...
 * a department is even chosen in the form; `SafeYouTubeTarget.departmentId`
 * lets the client filter down to whichever department is currently selected.
 */
export async function listAllYoutubeTargetsForTemplateFormAdmin(
  ...
  const targets = await listYoutubeTargets(actor);
